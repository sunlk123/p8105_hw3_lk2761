...
dataset and generates a table that shows the mean hour of the day at
which Pink Lady Apples and Coffee Ice Cream were ordered from Sunday to
Saturday.
...
``` r
# Create new dataset with only excellent responses for the Overall Health topic
...
ggplot(excellent_df, aes(group = state_name, x = year, y = avg_data_value, color = state_name)) + 
  ...
  theme_minimal() +
  theme(axis.title = element_text(size = 10)) +
  labs(title = "Distribution of the average value of excellent responses in states, 2002 through 2010", 
       x = "Year",
...
``` r
# Load and clean accelerometer data from imported csv
...
sums up the total daily activity for each of the 35
days.

``` r
# Create new dataframe and create new variable that sums up the activity counts per day 
...
# Create table that shows the day number, the name of the day of the week and the total number of activity counts for that day 
...
# Create a plot that shows the distribution of the 24-hour activity time courses for each day 
